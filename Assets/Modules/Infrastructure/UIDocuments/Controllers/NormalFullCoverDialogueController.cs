...
namespace Infrastructure.UIDocuments
{
  ...
  public class NormalFullCoverDialogueController : MonoBehaviour
  {
    public static NormalFullCoverDialogueController Instance { get; private set; }

    [SerializeField] float _characterIntervalSeconds = 0.03f;

    [SerializeField] UIDocument _uiDocument;
    [SerializeField] VisualElement _screenRoot;
    [SerializeField] NormalFullCoverDialogueElement _dialogueElement;
    ...
    [SerializeField] string _currentText = string.Empty;
    ...
    void CacheElements()
    {
      ...
      _dialogueElement = root?.Q<NormalFullCoverDialogueElement>(name: "dialogue-holder");
    }

    public Task Show()
    {
      if (_screenRoot == null || _dialogueElement == null)
        throw new InvalidOperationException("NormalFullCoverDialogueController is not initialized");
    ...
    public async Task PlayDialogue(string text, string presetText = null)
    {
      if (_dialogueElement == null || _screenRoot == null)
        throw new InvalidOperationException("NormalFullCoverDialogueController is not initialized");
      ...
      var preset = presetText ?? string.Empty;
      _dialogueElement.DialogueLabel.text = preset;
      ...
      await TypeTextAsync(_currentText, preset, _dialogueElement.DialogueLabel, _typingCts.Token);
    ...
    void SkipTyping()
    {
      if (_dialogueElement?.DialogueLabel == null) return;

      CancelTyping();
      _dialogueElement.DialogueLabel.text = _currentText;
      _isTyping = false;
      _typingCompleted = true;
      _dialogueElement.SetHintVisible(true);
    }
